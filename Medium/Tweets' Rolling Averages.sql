...
DataLemur Question Link: https://datalemur.com/questions/rolling-average-tweets
...
SELECT user_id, tweet_date, 
ROUND(AVG(tweet_count) OVER (PARTITION BY user_id ORDER BY tweet_date DESC
                        ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING),2) AS rolling_avg_3days
FROM tweetscount
