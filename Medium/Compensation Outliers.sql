...
DataLemur Question Link: https://datalemur.com/questions/compensation-outliers

Solution:

WITH AvgEmpSalary AS
(
SELECT title,
ROUND(AVG(salary)*2,0) AS DoubleAvgSalary,
ROUND(AVG(salary)/2,0) AS HalfAvgSalary
FROM employee_pay
GROUP BY title
),
...
statusbucket AS
(
SELECT ep.employee_id, ep.salary, ep.title,
CASE
  WHEN ep.salary > aes.DoubleAvgSalary THEN 'Overpaid'
  WHEN ep.salary < aes.HalfAvgSalary THEN 'Underpaid' END AS status
FROM employee_pay ep
INNER JOIN AvgEmpSalary aes
ON ep.title = aes.title
)

SELECT employee_id, salary, status FROM statusbucket
