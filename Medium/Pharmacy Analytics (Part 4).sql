...
DataLemur Question Link: https://datalemur.com/questions/top-drugs-sold

Solution:

SELECT manufacturer, top_drug FROM (
  SELECT manufacturer, drug AS top_drug, 
  RANK() OVER (PARTITION BY manufacturer ORDER BY SUM(units_sold) DESC) AS rankdrugs
  FROM pharmacy_sales
  GROUP BY manufacturer, drug
  ORDER BY manufacturer ASC) topdrugunits
WHERE rankdrugs <3