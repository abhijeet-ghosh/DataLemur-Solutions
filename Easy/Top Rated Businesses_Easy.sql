...
DataLemur Question Link : https://datalemur.com/questions/sql-top-businesses

Solution: 

SELECT 
  COUNT(CASE WHEN review_stars >=4 THEN 1 END) AS business_count,
  CAST(COUNT(CASE WHEN review_stars >=4 THEN 1 END) AS float) / COUNT(business_id)*100 AS top_rated_pct
FROM reviews