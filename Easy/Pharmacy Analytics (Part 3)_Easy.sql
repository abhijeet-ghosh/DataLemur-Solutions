...
DataLemur Question Link: https://datalemur.com/questions/total-drugs-sales

Solution:

SELECT manufacturer, sale FROM(
  SELECT manufacturer, SUM(total_sales) AS sales,
  CASE WHEN SUM(total_sales)>500000 THEN '$'|| (SUM(total_sales)/1000000.0)::numeric(10,0) || ' million' END AS sale
  FROM pharmacy_sales
  GROUP BY manufacturer) totalsales
ORDER BY sales DESC;