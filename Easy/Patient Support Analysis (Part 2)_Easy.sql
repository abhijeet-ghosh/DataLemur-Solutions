...
DataLemur Question Link: https://datalemur.com/questions/uncategorized-calls-percentage

Solution:

SELECT ROUND((uncat::decimal/tot_call)*100,1) AS call_percentage FROM 
(
SELECT 
COUNT(case_id) AS tot_call,
SUM(CASE WHEN call_category IS NULL OR call_category = 'n/a' THEN 1 ELSE 0 END) AS uncat
FROM callers
) uncat_callers