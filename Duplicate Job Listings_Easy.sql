...
DataLemur Question Link: https://datalemur.com/questions/duplicate-job-listings

Solution:

SELECT COUNT(dup_jobs) AS co_w_duplicate_jobs FROM(
SELECT COUNT(company_id) AS dup_jobs,company_id,title,description FROM job_listings
GROUP BY company_id,title,description
HAVING COUNT(company_id) > 1) duplicate_jobs