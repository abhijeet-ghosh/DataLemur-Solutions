...
DataLemur Question Link: https://datalemur.com/questions/sql-histogram-tweets

Solution: 

SELECT DISTINCT tweet_bucket, COUNT(users_num) AS "users_num"
FROM(
SELECT user_id, COUNT(user_id) AS "users_num", COUNT(tweet_id) AS tweet_bucket
FROM tweets
WHERE EXTRACT (YEAR FROM tweet_date) = 2022
GROUP BY user_id) usertweet
